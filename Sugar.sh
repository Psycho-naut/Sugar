...
EOF
		...
		then
		        mysql -e "use $DB; SELECT * FROM wifi WHERE SSID LIKE \"%$NAME%\" INTO OUTFILE './Export_WIFI_$NAME.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';"
	...
	then
                ...
                if [[ $INPUT == "Y" || $INPUT == "y" || $INPUT == "yes" ]];
                then
                        mysql -e "use $DB; SELECT * FROM bluetooth WHERE Device_Name LIKE \"%$NAME%\" INTO OUTFILE './Export_BL_$NAME.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';"
